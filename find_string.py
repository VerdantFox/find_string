...
def find_string(string_to_find, extension, directory, ignore_comments, case_insensitive):
    ...
    if extension != '' and extension[0] != '.':
        raise click.BadParameter("'{}' is not a recognized file extension".format(extension))
    if directory != '':
...
def traverse_files(dir_path, string_to_find, file_extension='', ignore_comments=False,
    ...
    for root, dirs, files in os.walk(dir_path):
        for file_name in files:
            if file_name.endswith(file_extension):
                ...
                search_file(file_path, string_to_find, ignore_comments, case_insensitive, dictionary)


def search_file(file_path, string_to_find, ignore_comments, case_insensitive, dictionary):
